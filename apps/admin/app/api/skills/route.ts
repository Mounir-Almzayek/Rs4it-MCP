...
export async function GET() {
  try {
    const res = await fetch(`${hubBaseUrl()}/api/skills`, { headers: hubHeaders(), cache: "no-store" });
    const payload = (await res.json()) as { ok?: boolean; skills?: unknown };
    const skills = Array.isArray(payload?.skills) ? payload.skills : [];
    return NextResponse.json(skills, { headers: NO_STORE });
  } catch (e) {
    console.error(e);
...
export async function PUT(request: NextRequest) {
  try {
    const body = (await request.json()) as { skills?: any[] };
    ...
    const res = await fetch(`${hubBaseUrl()}/api/skills`, {
      method: "PUT",
    ...
    if (!res.ok || payload?.ok === false) {
      return NextResponse.json({ error: String(payload?.error ?? "Failed to write skills") }, { status: 500 });
    }
    return NextResponse.json(payload.skills ?? [], { headers: NO_STORE });
  } catch (e) {
    console.error(e);
    return NextResponse.json({ error: "Failed to write skills" }, { status: 500 });
  }
}

...
export default function PermissionsMatrixPage() {
  ...
  const updateSkillRoles = useMutation({
    mutationFn: async ({ name, allowedRoles }: { name: string; allowedRoles: string[] | undefined }) => {
      const current = Array.isArray((registry as any)?.skills) ? ((registry as any).skills as any[]) : [];
  ...
  const updatePromptRoles = useMutation({
    mutationFn: async ({ name, allowedRoles }: { name: string; allowedRoles: string[] | undefined }) => {
      const current = Array.isArray((registry as any)?.prompts) ? ((registry as any).prompts as any[]) : [];

...
function useChartData(stats: UsageStats | undefined, range: "7d" | "30d" | "all") {
  return useMemo(() => {
    ...
    const timeSeries = Object.entries(byDay)
      ...
      .map(([date, count]) => ({ date, invocations: count, full: new Date(date).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" }) }));

    const entities = Object.entries(byEntity)
      .sort((a, b) => (b[1].total ?? 0) - (a[1].total ?? 0))
      .slice(0, 12)
      .map(([name, ent]) => ({ name: name.length > 20 ? name.slice(0, 18) + "…" : name, fullName: name, total: ent.total }));
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            Interactive charts and visual statistics for MCP usage.
          </p>
        </div>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <Link href="/usage" className="mt-4">
              <Button variant="outline">View Usage table</Button>
            </Link>
          </CardContent>
        </Card>
      )}
      ...
      {hasData && chartData && (
        <>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">Most invoked tools, skills, plugins (top 12)</p>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              ...
              <CardContent>
                {chartData.byUser.length === 0 ? (
                  <div className="flex h-[320px] items-center justify-center rounded-md border border-dashed text-muted-foreground text-sm">
                    No user data
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height={320}>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">Quick links</CardTitle>
            </CardHeader>

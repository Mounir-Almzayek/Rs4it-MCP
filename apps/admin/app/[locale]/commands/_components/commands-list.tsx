...
interface CommandsListProps {
  data: CommandRow[];
  isLoading: boolean;
  onEdit: (command: CommandRow) => void;
...
export function CommandsList({
  ...
  const columns: Column<CommandRow & Record<string, unknown>>[] = useMemo(
    () => [
      {
        key: "name",
        header: tc("name"),
      ...
      },
      {
        ...
        render: (row) => (
          <div className="flex items-center justify-end gap-1">
            <Button variant="ghost" size="icon" onClick={() => onEdit(row as CommandRow)} aria-label={tc("edit")}>
  ...
  return (
    <DataTable
      data={data as (CommandRow & Record<string, unknown>)[]}
      columns={columns}

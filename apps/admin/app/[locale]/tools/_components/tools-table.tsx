...
import type { ToolRow } from "../_hooks/use-tools";
...
export function ToolsTable({
  ...
  const columns: Column<ToolRow & Record<string, unknown>>[] = useMemo(
    () => [
      {
        key: "name",
        header: tc("name"),
          ...
          return (
            <div className="flex items-center justify-end gap-1">
              <Button
                variant="ghost"
                size="icon"
                onClick={() => onEdit(row as DynamicToolEntry)}
                aria-label={tc("edit")}
              >
  ...
  return (
    <DataTable
      data={data as (ToolRow & Record<string, unknown>)[]}
      columns={columns}
      keyFn={(row) =>
        row.source === "mcp"
          ? `${row.origin}:${row.name}`
      ...
      searchFields={["name", "description", "handlerRef"]}
      emptyIcon={Wrench}

...
export function PromptsList({
  ...
  const columns: Column<PromptRow & Record<string, unknown>>[] = useMemo(
    () => [
      {
        key: "name",
        header: tc("name"),
      ...
      },
      {
        ...
        render: (row) => (
          <div className="flex items-center justify-end gap-1">
            <Button variant="ghost" size="icon" onClick={() => onEdit(row as PromptRow)} aria-label={tc("edit")}>
  ...
  return (
    <DataTable
      data={data as (PromptRow & Record<string, unknown>)[]}
      columns={columns}

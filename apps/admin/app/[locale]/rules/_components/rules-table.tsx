...
interface RulesTableProps {
  data: DynamicRuleEntry[];
  roles: RoleDefinition[];
  isLoading: boolean;
  onEdit: (rule: DynamicRuleEntry) => void;
...
export function RulesTable({
  ...
  const columns: Column<DynamicRuleEntry & Record<string, unknown>>[] =
    useMemo(
      () => [
        {
          key: "name",
  ...
  return (
    <DataTable
      data={data as (DynamicRuleEntry & Record<string, unknown>)[]}
      columns={columns}
      keyFn={(row) => String(row.name)}
      searchFields={["name", "description", "globs"]}
      emptyIcon={Scale}
